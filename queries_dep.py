...
def performances_dep(annee, mois_int, lib_departement):
    ...
    """
    result = pd.read_sql(query.format(annee, mois_int, lib_departement), conn).values
    conn.close()
...
def performances_dep_item(annee, mois_int, lib_departement, id_famille_produit) :
    conn = sqlite3.connect('data.db')
